...
- Passwordless authentication using a verified mobile number belonging to a family manager or Young Adult Scout and an SMS one-time code or magic link
- Family accounts managed by one or more parents/guardians, without requiring scouts to have phones or login credentials
- Optional Young Adult Scout access for older scouts with their own phones, limited to their own schedule and shift actions
...
- HTMX may be used for progressive enhancement and partial-page updates
...
- Core workflows remain usable with standard browser navigation if JavaScript or HTMX is unavailable
...
**Description:** Before the system can be used, the very first administrator login must be established. This is a one-time setup that occurs when the system is initially deployed. Since the system uses passwordless phone authentication and all family accounts require invitations generated by an admin, a special bootstrap process is needed to create that first admin.

**Preconditions:**
- The Go web application and SMS authentication provider have been deployed and configured
- No administrator accounts exist in the system yet
- The designated first admin's mobile phone number is known

**What Happens:**
1. During initial system deployment, the designated first admin's phone number is registered as the bootstrap administrator
2. The designated person opens the web app, enters that phone number, and requests an SMS one-time code or magic link
3. The system verifies the code or link, recognizes the bootstrap phone number, and grants administrator privileges
...
- The bootstrap phone number must be configured before the system goes live
...
- SMS codes and links are short-lived, single-use, rate-limited, and never reveal whether an unregistered phone number exists
...
**What Happens:**
1. Admin generates a unique, expiring invitation for the family and sends it to the primary parent's mobile phone number
...
**Description:** A family manager or Young Adult Scout accesses their permitted family-account functions without a password or third-party social login.

**What Happens:**
1. The person opens the web app on any supported browser
2. They enter their registered mobile phone number
3. The system sends a short-lived SMS one-time code or magic link
4. They complete verification in the browser
5. The system creates a secure session and applies permissions for their role
6. A Family Manager opens the family dashboard; a Young Adult Scout opens their personal schedule
7. Unless the person's own current-season agreement status is Confirmed, the web app prominently directs them to the Agreement Center and disables their participation actions

**Security and Recovery:**
- Codes and links expire quickly, are single-use, and are rate-limited
...
- Losing access to a registered phone number requires identity verification by an administrator; a family manager may instead revoke and reissue a Young Adult Scout's access to a newly verified number
...
**Description:** A Family Manager or Admin grants limited authenticated access to an older scout who has their own phone and is ready to manage their own shift participation.
...
2. The authorized user enters the scout's own mobile phone number
3. The system sends a unique, expiring invitation by SMS
4. The scout opens the invitation, verifies the phone number, and accepts access
...
- Accepting the invitation creates an authenticated identity for the verified phone number and links it to the existing scout profile
- The scout's family membership continues to come from the profile's family/household relationships, not from the phone number
...
- A verified phone number can belong to only one authenticated person in the entire system
- The invitation is rejected if the phone number is already linked to a Family Manager, Young Adult Scout, Committee Member, or Admin
- A phone number cannot silently create or select a different profile; conflicts require administrator resolution
...
### Use Case 2B: Authenticated Person Changes Phone Number

**Actors:** Family Manager, Young Adult Scout, Committee Member, Admin

**Description:** An authenticated person replaces their login phone number without creating a new identity or losing family relationships, roles, assignments, or history.

**Standard Process (old number still available):**
1. The person opens account security settings and selects "Change Phone Number"
2. The system requires recent authentication using the current phone number
3. The person enters the new phone number
4. The system normalizes the new number and confirms that it is not linked to another identity
5. The system sends a one-time verification code or magic link to the new number
6. After successful verification, the system atomically replaces the phone number on the existing authenticated identity
7. All existing browser sessions are revoked, and the person signs in again using the new number
8. A security notice is sent to both the old and new numbers

**Recovery Process (old number unavailable):**
1. The person contacts an authorized recovery actor without attempting to create a second account
2. For a Young Adult Scout, a Family Manager may revoke the old access and reissue it to the scout's new verified number
3. For a Family Manager with an active co-manager, the co-manager or an Admin may revoke and reissue access after confirming the person's identity
...
6. The old phone link and all sessions are revoked before the new number is linked
7. The new number must pass uniqueness checks and SMS verification

**Failure and Security Rules:**
- If the new number is already linked to another person, the change is rejected and the old login remains unchanged
- Possession of the new number alone is not sufficient to take over an existing identity
- The old number stops working immediately after a successful change
- The existing internal user ID is preserved; this is a credential change, not an account migration
- Recovery actions are recorded in the audit trail

**Outcome:** The person signs in with the new number while retaining the same profile, family memberships, permissions, assignments, and historical records.
...
**Description:** A second parent or guardian is invited to share family management responsibilities using their own verified phone number.

**What Happens:**
1. The existing manager adds the co-parent/guardian as an adult family member and enters their mobile phone number
2. The system sends a unique, expiring manager invitation by SMS
3. The co-manager opens the web app and verifies that phone number with an SMS code or magic link
4. The co-manager reviews and accepts access to the family account
5. The co-manager gains family management permissions through their own login
...
**What Happens:**
1. The scout signs in using their own verified phone number
...
4. Admin re-authenticates using SMS, confirms that the archive and passphrase have been saved separately, and types the season name
...
**Note:** Authentication is tied to a verified mobile phone number rather than an email or social-login account. Phone-number changes and recovery follow Use Case 2B, including recovery by an authorized Family Manager, co-manager, Admin, or secured break-glass process as appropriate.
...
3. The person reviews the consequences and re-authenticates by SMS
4. On confirmation, the system:
   - Revokes all sessions for that manager
   - Removes the verified phone number from active authentication and notification delivery
...
2. System compiles all data associated with the user:
   - Profile information (name, phone number if still present, photo)
...
- Core workflows remain usable through standard browser requests without a heavy client-side framework
...
- **Co-Manager Invitation:** Created by an existing Family Manager or Admin and sent to another parent/guardian's phone number to grant management of an existing household
- **Young Adult Scout Invitation:** Sent to an older scout's own phone number to grant limited access to their existing profile
- **Household Link Code:** Used by a family manager to add an existing scout profile to an additional household
...
- Manager and Young Adult Scout invitations are bound to the intended normalized phone number; a Household Link Code is bound to one scout profile
...
- Each authenticated person uses their own verified mobile phone number
- A normalized phone number is system-wide unique and can be linked to only one active authenticated identity at a time
- Two people cannot share a login phone number, even when they belong to the same family
- The same person uses one identity if they hold multiple roles, such as Family Manager and Committee Member
- A phone number may be reassigned to another person only after its previous identity link and sessions are explicitly revoked and the new owner verifies the number
- Conflict messages do not reveal the identity or family currently associated with a phone number
- Sign-in uses a short-lived, single-use SMS one-time code or magic link; Google, Apple, and other social identity providers are not used
- Authentication responses are rate-limited and do not disclose whether a phone number is registered
...
- A self-service phone-number change requires recent verification of the old number and verification of the unique new number
- If the old number is unavailable, recovery follows Use Case 2B: a Family Manager may recover a Young Adult Scout, a co-manager or Admin may recover a Family Manager, and another Admin or the secured break-glass process may recover an Admin
- Changing a phone number updates the existing identity; it never creates a replacement person profile or discards roles, memberships, assignments, or history
...
- Scouts do not require a phone number or authenticated login; Young Adult Scout access is optional
...
- Users never share login codes or browser sessions; actions are attributed to the authenticated person who performed them
...
- Successful phone verification links an authenticated identity to that profile; it does not create a second family member
...
- Authentication, invitation, recovery, and security-alert SMS messages are transactional and never controlled by notification preferences
...
**Mobile Phone Number:**
- Each authenticated user's login is tied to a verified mobile phone number
- Phone numbers are converted to one canonical international form (for example, E.164) before comparison and must be unique across all active authenticated identities
- A phone number cannot be used by multiple Family Managers, Young Adult Scouts, Committee Members, or Admins
- The number is used for passwordless sign-in and may be used for operational SMS notifications
- Changing a phone number updates the existing authenticated identity and preserves its internal user ID, roles, family memberships, and history
- The standard change flow requires recent authentication with the old number and verification of the new number
- After a successful change, all sessions are revoked and security notices are sent to both the old and new numbers
- If the old number is unavailable, authorized recovery must revoke the old link and sessions before verifying and linking the replacement
...
- A login phone number is visible only to its owner, authorized Family Managers for that profile, and Admins performing support or recovery; it is not displayed on rosters, leaderboards, or to unrelated households
...
| Data Element | Action |
|--------------|--------|
| Verified phone authentication | Removed |
...
| SMS notification destination | Removed |
...
1. Invite another adult who verifies their phone number and accepts co-manager access
