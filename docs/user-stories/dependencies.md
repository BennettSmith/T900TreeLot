...
- **Communications and outbox:** Canonical in-app inbox records and optional Groups.io deliveries are independent. Inbox records and outbox entries commit with the domain change; provider calls happen afterward and are idempotent. Groups.io is disabled by default, limited to troop-wide messages, and never blocks in-app publication. Direct and family-scoped messages have no Groups.io edge; verified-email delivery is a later follow-on and is not a hard prerequisite for current communication stories.
