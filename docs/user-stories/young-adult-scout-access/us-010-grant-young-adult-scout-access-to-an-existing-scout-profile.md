...
**As a** Family Manager, **I want** to grant an older scout limited access through their own verified phone number, **so that** they can manage their own participation without gaining authority over anyone else.

## Scope

Authorize access from an existing scout profile, send a profile-specific invitation to the scout's own phone number, verify acceptance, and link one authenticated identity to that profile with Young Adult Scout permissions.
...
- The scout has their own mobile phone number.

## Acceptance criteria

1. **Given** an authorized Family Manager or Admin starts from an existing scout profile and enters the scout's own number, **when** access is granted, **then** the system sends a unique, single-use, expiring Young Adult Scout invitation bound to that profile, purpose, and normalized number.
2. **Given** the scout opens a valid invitation, **when** they verify the bound number and accept, **then** the system links a new authenticated identity to the existing scout profile without creating another family member.
3. **Given** the normalized number is already linked to a Family Manager, Young Adult Scout, Committee Member, or Admin identity, **when** acceptance is attempted, **then** the invitation is rejected and cannot silently create or select a different profile.
...
- A verified normalized phone number belongs to only one active identity system-wide.
...
- US-002 provides passwordless verification and secure identity sessions.
...
- Granting access to a phone number already linked to another identity.
